...
class YOLOSegmentationLoss:
    ...
    def _process_segments_to_masks(
        ...
        for i, segments in enumerate(segments_batch): # Iterate through images in batch
            ...
            matched_segment_indices = [-1] * len(valid_bboxes) # Index of segment for each bbox

            if segments: # Check if there are any segments for this image
                for seg_idx, segment in enumerate(segments):
                    poly = np.array(segment[1:]).reshape(-1, 2) # Polygon points (normalized)
                    
                    # Calculate segment center (approximate)
                    seg_center_x, seg_center_y = poly.mean(axis=0)

                    # Find the closest valid bbox center
                    distances = (centers_x - seg_center_x)**2 + (centers_y - seg_center_y)**2
                    best_match_bbox_idx = distances.argmin()

                    # Simple check: if not already matched or this segment is closer
                    if matched_segment_indices[best_match_bbox_idx] == -1 or \
                       distances[best_match_bbox_idx] < distances[matched_segment_indices[best_match_bbox_idx]]:
    ...
    def __call__(self, predicts, targets_bboxes, targets_segments):
        ...
        targets_cls_aligned, targets_bbox_aligned_scaled = torch.split(align_targets, (self.class_num, 4), dim=-1) 
        
        # Un-scale the bounding boxes for loss calculation (using original vec2box scaler logic)
        vec2box = self.matcher.vec2box 
        # Note: We divide by scaler here because BoxLoss/DFLoss expect UN-SCALED target boxes relative to anchor grid
        targets_bbox_aligned = targets_bbox_aligned_scaled / vec2box.scaler[None, :, None] 
